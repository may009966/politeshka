...
public class Controller {
    ...
    public void initialize(){
        ...
        scheduler.add(new MemoryBlock(0, Utils.maxMemorySize-148, Utils.maxMemorySize));
        ...
        new Thread() {
            ExecutingTask executingTask = new ExecutingTask(processes);
        };

        new Thread() {
            ExecutingWork executingWork = new ExecutingWork(processes);
        };

        new Thread() {
            ExecutingCheckTime executingCheckTime = new ExecutingCheckTime(processes);
        };

        Thread thread = new Thread() {
            ExecutingCheck executingCheck = new ExecutingCheck(processes);
        };

        new Thread() {
            ExecutingRefreshData executingRefreshData = new ExecutingRefreshData();
        };
    ...
    public static void Refresh(){
        ...
        for (Process process : processes.getList()) {
            if (process.getTypeState().equals(State.READY) || process.getTypeState().equals(State.RUNNING) || process.getTypeState().equals(State.TERMINATED)) {
        ...
        for (Process process : processes.getList()) {
            if (process.getTypeState().equals(State.REJECTED)) {
        ...
        TimerTask timerTask = new TimerTask() {
            ...
            public void run() {
                if (!processes.getList().isEmpty()) {
                    ...
                    int memory = 0;
                    for (Process process:processes.getList()) {
                        if(process.getTypeState().equals(State.WAITING)){
                            quantityInPending++;
                        } else if(process.getTypeState().equals(State.TERMINATED)){
                            quantityTerminated++;
                        }
                    }
